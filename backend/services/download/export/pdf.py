...
def _metadata_table(metadata: dict[str, Any]) -> Table:
    ...
    # Only present when a depth-resolved variable was requested. Both figures
    # are shown because they differ: the model's levels are geometrically
    # spaced, so a request for 100m resolves to 92.3m.
    if "resolved_depth_m" in metadata:
        rows.append(
            [
                "Depth",
                Paragraph(
                    f"{metadata['resolved_depth_m']} m "
                    f"(nearest model level to the requested {metadata['requested_depth_m']} m)",
                    _CELL_STYLE,
                ),
            ]
        )
...
def to_pdf_bytes(
    ...
    if numeric_codes:
        ...
        # A time-invariant request (bathymetry alone) collapses to a single
        # timestamp, where a "trend over time" line is a single point and
        # actively misleading. The distribution across space still says
        # something, so that one is always drawn.
        has_time_series = df["timestamp"].nunique() > 1
    ...
    story.append(Spacer(1, 0.15 * inch))
    # One licence paragraph per provider family actually used — a request can
    # now mix Copernicus, Open-Meteo and GEBCO, which are under three
    # different terms.
    for licence in metadata.get("licences", []):
        story.append(Paragraph(licence, _STYLES["Normal"]))
        story.append(Spacer(1, 0.1 * inch))
    story.append(Paragraph("Generated by MarisAI.", _STYLES["Normal"]))
